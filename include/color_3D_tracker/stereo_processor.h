#ifndef STEREO_IMAGE_PROCESSOR_H_
#define STEREO_IMAGE_PROCESSOR_H_
 ...
 * To use this class, subclass it and implement the stereoImageCallback() method.
 */
...
private:
              ...
              const sensor_msgs::CameraInfoConstPtr& r_info_msg)
  {
    ...
    // call implementation
    stereoImageCallback(l_image_msg, r_image_msg, l_info_msg, r_info_msg);
  }
...
protected:
    ...
    left_received_(0), right_received_(0), left_info_received_(0), right_info_received_(0), all_received_(0)
  {
    ...
    left_sub_.registerCallback(boost::bind(StereoImageProcessor::increment, &left_received_));
    right_sub_.registerCallback(boost::bind(StereoImageProcessor::increment, &right_received_));
    left_info_sub_.registerCallback(boost::bind(StereoImageProcessor::increment, &left_info_received_));
    right_info_sub_.registerCallback(boost::bind(StereoImageProcessor::increment, &right_info_received_));
    check_synced_timer_ = nh.createWallTimer(ros::WallDuration(15.0),
                                             boost::bind(&StereoImageProcessor::checkInputsSynchronized, this));
    ...
    if (approx)
    {
      ...
      approximate_sync_->registerCallback(boost::bind(&StereoImageProcessor::dataCb, this, _1, _2, _3, _4));
    }
    else
    {
      exact_sync_.reset(new ExactSync(ExactPolicy(queue_size_),
                                      left_sub_, right_sub_, left_info_sub_, right_info_sub_) );
      exact_sync_->registerCallback(boost::bind(&StereoImageProcessor::dataCb, this, _1, _2, _3, _4));
    }
  }
