...
/** \brief Trainer constructor
  * \param transport
  */
color_3D_tracker::Trainer::Trainer(const string transport) : StereoImageProcessor(transport)
{
  ROS_INFO_STREAM("[Trainer:] Instantiating the color_circle Trainer...");
  ...
  ROS_INFO_STREAM("[Trainer:] color_circle Trainer Settings:" << endl <<
                  "  trained_model_path   = " << trained_model_path_ << endl <<
                  "  num_hue_bins         = " << num_hue_bins_ << endl <<
                  "  num_sat_bins         = " << num_sat_bins_ << endl <<
                  "  num_val_bins         = " << num_val_bins_ << endl);

  // first status: show live video
  training_status_ = DISPLAY_VIDEO;

  // OpenCV image windows for debugging
  namedWindow("Training GUI", 0);
  setMouseCallback("Training GUI", &color_3D_tracker::Trainer::staticMouseCallback, this);
}

color_3D_tracker::Trainer::~Trainer()
{
  destroyWindow("Training GUI");
}
...
/** \brief Stereo Image Callback
  ...
  */
void color_3D_tracker::Trainer::stereoImageCallback(
  ...
  const sensor_msgs::CameraInfoConstPtr& r_info_msg)
{
  ...
  switch(training_status_)
  {
    ...
    case PAINTING:
      ...
      image = training_image_.clone();
      rectangle(image, roi_rectangle_selection_, Scalar(255,255,255),3);
      break;
    case ROI_SELECTED:
    ...
    case TRAINED:
      ...
      FileStorage fs(trained_model_path_, FileStorage::WRITE);
...
/** \brief Detect the color_circle into the image
  ...
  */
MatND color_3D_tracker::Trainer::train(const Mat& image)
{
  ...
  Mat roi(hsv_image, roi_rectangle_selection_);
  ROS_INFO("Extracting histogram from target...");
  MatND target_hist = calculateHistogram(roi, bins_hsv, Mat());

  // Calculate the histogram of the background
  Mat maskroi;
  maskroi = Mat::zeros(hsv_image.size(), CV_8UC1); 
  rectangle(maskroi, roi_rectangle_selection_, 255);
  ROS_INFO("Extracting histogram from background...");
  MatND background_hist = calculateHistogram(hsv_image, bins_hsv, maskroi);

  //MatND model_histogram = target_hist / background_hist * 255;
  MatND model_histogram = target_hist;
  //MatND model_histogram(target_hist.size(), CV_32FC3);
  float epsilon = 1e-6;
  ...
  namedWindow("color_circle-HS-histogram", 0);
  namedWindow("color_circle-HV-histogram", 0);
  showHSVHistogram(model_histogram,"color_circle-HS-histogram","color_circle-HV-histogram");
...
/** \brief static function version of mouseCallback
  ...
  */
void color_3D_tracker::Trainer::staticMouseCallback(int event, int x, int y, int flags, void* param)
{
...
/** \brief Mouse callback for training
  ...
  * \param input params. Need to be correctly re-caster to work
  */
void color_3D_tracker::Trainer::mouseCallback( int event, int x, int y, int flags, void* param)
{
  ...
  switch( training_status_ )
  {
    ...
    case SHOWING_TRAINING_IMAGE:
      ...
      if (event == CV_EVENT_LBUTTONDOWN)
      {
        roi_rectangle_origin_ = Point(x,y);
        roi_rectangle_selection_ = Rect(x,y,0,0);
    ...
    case PAINTING:
      ...
      roi_rectangle_selection_.x = min(x, roi_rectangle_origin_.x);
      roi_rectangle_selection_.y = min(y, roi_rectangle_origin_.y);
      roi_rectangle_selection_.width = abs(x - roi_rectangle_origin_.x);
      roi_rectangle_selection_.height = abs(y - roi_rectangle_origin_.y);
      roi_rectangle_selection_ &= Rect(0, 0, image.cols, image.rows);
...
/** \brief Histogram calculation in HSV colorspace
  ...
  */
MatND color_3D_tracker::Trainer::calculateHistogram(const Mat& image, 
                                                          const int bins[],  
                                                          const Mat& mask)
{   
  ...
  int num_arrays = 1;
  int dimensions = 3;
  calcHist(&image, num_arrays, channels, mask, histogram, dimensions, bins, ranges_hsv);
...
/** \brief Histogram plot in OpenCV namedWindow
  ...
  */
void color_3D_tracker::Trainer::showHSVHistogram(const MatND& histogram,
                                                   ...
                                                   const string& name_hv)
{
  ...
  Mat histogram_image_hs = Mat::zeros((num_saturation_bins + 1) * scale, (num_hue_bins + 1) * scale, CV_8UC3);
  Mat histogram_image_hv = Mat::zeros((num_value_bins + 1) * scale, (num_hue_bins + 1) * scale, CV_8UC3);

  // X axis
  for( int h = 1; h < num_hue_bins + 1; h++ )
    rectangle( histogram_image_hs, Point(h*scale, 0),
                   Point((h + 1)*scale - 1, scale - 1),
                   Scalar(1.0 * (h - 1) / num_hue_bins * 180.0, 255, 255, 0),
                   CV_FILLED);

  // Y axis
  for( int s = 1; s < num_saturation_bins + 1; s++ )
    rectangle( histogram_image_hs, Point(0, s * scale),
                   Point(scale - 1, (s + 1)*scale - 1),
                   Scalar(180, 1.0 * (s - 1) / num_saturation_bins * 255.0, 255, 0),
                   CV_FILLED);
  // second X axis
  for( int h = 1; h < num_hue_bins + 1; h++ )
    rectangle( histogram_image_hv, Point(h*scale, 0),
                   Point((h + 1)*scale - 1, scale - 1),
                   Scalar(1.0 * (h - 1) / num_hue_bins * 180.0, 255, 255, 0),
                   CV_FILLED);
  ...
  // second Y axis
  for( int v = 1; v < num_value_bins + 1; v++ )
    rectangle( histogram_image_hv, Point(0, v * scale),
                   Point(scale - 1, (v + 1)*scale - 1),
                   Scalar(180, 255, 1.0 * (v - 1) / num_value_bins * 255.0,  0),
                   CV_FILLED);

  Mat histogram_hs, histogram_hv;
  cvtColor(histogram_image_hs, histogram_hs, CV_HSV2BGR);
  cvtColor(histogram_image_hv, histogram_hv, CV_HSV2BGR);
  ...
  for( int h = 1; h < num_hue_bins + 1; h++ )
    for( int s = 1; s < num_saturation_bins + 1; s++ )
    {
      ...
      rectangle( histogram_hs, Point(h*scale, s*scale),
  ...
  for( int h = 1; h < num_hue_bins + 1; h++ )
    for( int v = 1; v < num_value_bins + 1; v++ )
    {
      ...
      rectangle( histogram_hv, Point(h*scale, v*scale),
                     Point( (h+1)*scale - 1, (v+1)*scale - 1),
                     Scalar::all(intensity),
                     CV_FILLED );
     }
  imshow( name_hs, histogram_hs );
  imshow( name_hv, histogram_hv );
  waitKey(5);
}